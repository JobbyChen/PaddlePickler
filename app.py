...
# Keep this CSV file in the same GitHub repo folder as app.py.
CSV_FILENAME = "paddles_balanced_90_image_ready.csv"

# Optional fallback names in case your finished CSV has a different name.
CSV_FALLBACKS = [
    "paddles_balanced_90_scraped_images.csv",
    "paddles_balanced_90_real_links.csv",
    "paddles_balanced_90.csv",
]
...
def get_image_search_link(paddle):
    ...
    search_query = quote_plus(paddle.name + " pickleball paddle front image")
    return f"https://www.google.com/search?tbm=isch&q={search_query}"


def looks_like_side_view(url):
    if not url:
        return False
    ...
    return any(keyword in url for keyword in side_keywords)


def looks_like_bad_front_view(url):
    if not url:
        return False
    ...
    return any(keyword in url for keyword in bad_front_keywords)


def choose_front_image(paddle):
    # Prefer verified front image column.
    if paddle.front_image_url and not looks_like_bad_front_view(paddle.front_image_url):
        return paddle.front_image_url

    # Then try the general image column.
    if paddle.image_url and not looks_like_bad_front_view(paddle.image_url):
        return paddle.image_url

    # If the only available image looks like a side/detail view, do not use it as front.
    return ""


def choose_side_image(paddle, front):
    ...
    # Trust side_image_url because this column is meant to be manually/explicitly verified.
    # We do NOT use image_url as a side image, so this avoids accidental duplicate front views.
    return side


def show_paddle_images(paddle):
    front = choose_front_image(paddle)
    side = choose_side_image(paddle, front)
    ...
    if front and side:
        ...
        st.caption("Front / product view")
        st.image(front, use_container_width=True)
        st.caption("Side view not available for this paddle yet.")

    else:
        st.info("No verified front image yet.")
        st.markdown("## 🏓")
        st.caption("Add a better URL in the front_image_url column for this paddle.")
...
def main():
    ...
    if not csv_file:
        st.error(
            "Could not find the paddle CSV. Upload "
            "`paddles_balanced_90_image_ready.csv` with this app.py file."
        )
        return

    paddles = load_paddles_from_csv(csv_file)
    ...
    handle_filter = st.sidebar.selectbox(
        "Only show handle category",
        ["Any", "Short", "Standard", "Long"]
    )

    weight_filter = st.sidebar.selectbox(
        "Only show weight category",
        ["Any", "Light", "Medium", "Heavy"]
    )

    price_filter = st.sidebar.selectbox(
        "Only show price category",
        ["Any", "Budget", "Mid-range", "Premium"]
    )

    style_filter = st.sidebar.selectbox(
        "Only show style tag",
        ["Any", "Control", "Power", "Spin", "All-court", "Singles", "Doubles"]
    )
    ...
    for index, item in enumerate(recommendations, start=1):
        ...
        with right_col:
            ...
            with button_col2:
                st.link_button("Find Better Image", get_image_search_link(paddle))
